tseriescm <-
function(filedir,maxiter=2000,p=1,d=13,c0eps=2,c1eps=1,
  ...
  if(p%%1 != 0 | d%%1 != 0 | p <= 0 | d < 11){
    stop("p and d must be positive integer numbers and d must be greater than or equal to 11.")
  }
  ...
  if (p > 1){                              # The excluded parameters are the level and the trend terms
    ...
    if(d > 11){
      X1 <- matrix(0,T,(d-11))               # Columns for the trend terms in the design matrix. 
  ...
  if (p == 1){                             # The only excluded parameter is the level of the data
    ...
    if (num < 1){                            # If the number of months in the data is less than 12, the design matrix is filled this way
      ...
      X2 <- rbind(X2,X21[0:resid,])            # Construction of the month indicator matrix according to how many years are present in the data
